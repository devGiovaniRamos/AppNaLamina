  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-serif font-semibold text-stone-50">Estoque</h1>
          <p className="text-stone-400 text-sm mt-1">{produtosFiltrados.length} de {produtos.length} produto(s) ativo(s)</p>
        </div>
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <input
          ...
          className="w-full border border-stone-700 rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500"
        />
      </div>

      {/* Filtro de data (para entradas/saídas do período) */}
      <div className="flex items-center gap-3 mb-6">
        <span className="text-sm text-stone-400">Período das movimentações:</span>
      ...
      {relatorio && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
          <div className="bg-stone-900 rounded-xl border border-stone-800 shadow-sm p-4">
            ...
            <p className="text-xl font-bold text-stone-50">{relatorio.totalProdutosAtivos}</p>
          </div>
          <div className="bg-stone-900 rounded-xl border border-stone-800 shadow-sm p-4">
            <div className="flex items-center gap-2 label-caps mb-2">
              <AlertTriangle size={14} /> Estoque baixo
            </div>
            <p className={`text-xl font-bold ${relatorio.produtosComEstoqueBaixo > 0 ? 'text-red-400' : 'text-stone-50'}`}>
          ...
          <div className="bg-stone-900 rounded-xl border border-stone-800 shadow-sm p-4">
            ...
            <p className="text-xl font-bold text-stone-50">{fmt(relatorio.valorTotalEstoque)}</p>
          </div>
          <div className="bg-stone-900 rounded-xl border border-stone-800 shadow-sm p-4">
            <div className="flex items-center gap-2 label-caps mb-2">
              <TrendingUp size={14} /> Entradas no período
            </div>
            <p className="text-xl font-bold text-stone-50">{Number(relatorio.totalEntradas || 0).toFixed(2)}</p>
          </div>
          <div className="bg-stone-900 rounded-xl border border-stone-800 shadow-sm p-4">
            <div className="flex items-center gap-2 label-caps mb-2">
              <TrendingDown size={14} /> Saídas no período
            </div>
            <p className="text-xl font-bold text-stone-50">{Number(relatorio.totalSaidas || 0).toFixed(2)}</p>
          </div>
        </div>
      )}

      {relatorio?.produtosBaixoEstoque?.length > 0 && (
        <div className="flex items-start gap-2 mb-6 bg-red-500/10 border border-red-500/30 text-red-400 text-sm rounded-lg px-4 py-3">
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-800/60 border-b border-stone-800">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-stone-400 font-medium">Categoria</th>
                <th className="text-left px-4 py-3 text-stone-400 font-medium">Uso</th>
                <th className="text-left px-4 py-3 text-stone-400 font-medium">Estoque</th>
                <th className="text-left px-4 py-3 text-stone-400 font-medium">Preço venda</th>
            ...
            <tbody className="divide-y divide-stone-800">
              {produtosFiltrados.map(p => (
                <tr key={p.id} className="hover:bg-stone-800/60 transition-colors">
                  ...
                  <td className="px-4 py-3 text-stone-300">{CATEGORIA_LABEL[p.categoria] || p.categoria}</td>
                  <td className="px-4 py-3 text-stone-300">{TIPO_USO_LABEL[p.tipoUso] || p.tipoUso}</td>
                  <td className="px-4 py-3">
                    <span className={p.estoqueBaixo ? 'text-red-400 font-medium' : 'text-stone-300'}>
                      {Number(p.estoqueAtual).toFixed(2)} {UNIDADE_LABEL[p.unidadeMedida] || p.unidadeMedida}
                    </span>
                    <span className="text-stone-500 text-xs"> (mín. {Number(p.estoqueMinimo).toFixed(2)})</span>
                  </td>
                  <td className="px-4 py-3 text-stone-300">{p.precoVenda != null ? fmt(p.precoVenda) : '—'}</td>
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <button onClick={() => abrirAjuste(p, 'ENTRADA')} title="Entrada de estoque"
                      className="p-1.5 text-emerald-400 hover:bg-emerald-500/10 rounded-lg mr-1">
                      <PackagePlus size={15} />
                    </button>
                    ...
                    <button onClick={() => abrirEditar(p)} title="Editar"
                      className="p-1.5 text-stone-500 hover:text-gold-400 hover:bg-gold-500/10 rounded-lg mr-1">
                      <Pencil size={15} />
                    </button>
                    <button onClick={() => handleDesativar(p.id)} title="Desativar"
                      className="p-1.5 text-stone-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg">
                      <Trash2 size={15} />
                    </button>
                  </td>
                </tr>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editando ? 'Editar Produto' : 'Novo Produto'}>
        <form onSubmit={handleSalvar} className="space-y-4">
          {editando && (
            <p className="text-xs text-stone-500">SKU: <span className="font-mono">{editando.sku}</span> (gerado automaticamente)</p>
          )}
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-stone-200 mb-1">EAN (código de barras)</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-stone-200 mb-1">Categoria *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-stone-200 mb-1">Tipo de uso *</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-stone-200 mb-1">Unidade *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-stone-200 mb-1">Estoque atual *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-stone-200 mb-1">Estoque mínimo *</label>
              <input type="number" className="input" required min={0} step={0.01} value={form.estoqueMinimo}
                onChange={e => setForm({ ...form, estoqueMinimo: e.target.value })} />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-stone-200 mb-1">Preço de venda (R$)</label>
      ...
      <Modal open={!!modalAjuste} onClose={fecharAjuste} title={tipoAjuste === 'ENTRADA' ? 'Entrada de Estoque' : 'Saída de Estoque'}>
        {modalAjuste && (
          <form onSubmit={handleAjustar} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium text-stone-200 mb-1">Quantidade *</label>
            ...
            {erroAjuste && <p className="text-red-400 text-sm bg-red-500/10 p-2 rounded-lg">{erroAjuste}</p>}
            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={fecharAjuste} className="btn-ghost">Cancelar</button>
              <button type="submit" disabled={ajustando}
                className={`px-4 py-2 text-sm text-white rounded-lg disabled:opacity-50 ${tipoAjuste === 'ENTRADA' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-orange-600 hover:bg-orange-700'}`}>
